...
function renderBundle(bundle) {
  ...
  const draw=()=>{
    ...
    const galleryCandidates=[
      {url:active.image_url,label:active.label_ar||active.label_en||bundle.name_ar},
      ...
      ...options.map(option=>({url:option.image_url,label:option.label_ar||option.label_en||bundle.name_ar}))
    ];
    ...
    const showMedia=index=>{if(!gallery.length)return;mediaIndex=(Number(index)+gallery.length)%gallery.length;const image=document.getElementById("bundleGalleryImage"),item=gallery[mediaIndex];if(image){image.src=mediaPreviewUrl(item.url,960);image.alt=item.label||bundle.name_ar||"";}document.querySelectorAll("[data-bundle-gallery-index]").forEach((button,buttonIndex)=>button.classList.toggle("active",buttonIndex===mediaIndex));document.querySelector(`[data-bundle-gallery-index="${mediaIndex}"]`)?.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"});};
    const openBundleImage=()=>{const item=gallery[mediaIndex];if(!item)return;openOverlay(`<div class="lightbox" role="dialog" aria-modal="true" aria-label="صورة الطقم الأصلية"><button class="close-button" data-overlay-close aria-label="إغلاق">${icon("x")}</button><div class="lightbox-media"><span class="lightbox-loading" role="status">جاري تحميل الصورة الأصلية...</span><img src="${esc(item.url)}" alt="${esc(item.label||bundle.name_ar||"")}" style="display:none" /></div></div>`);const image=overlayRoot.querySelector(".lightbox-media img"),show=()=>{image.style.display="block";overlayRoot.querySelector(".lightbox-loading")?.remove();};image.onload=show;image.onerror=()=>{overlayRoot.querySelector(".lightbox-loading").textContent="تعذر تحميل الصورة";};if(image.complete&&image.naturalWidth)show();hydrateIcons();};
    document.getElementById("bundleGalleryImage")?.addEventListener("click",openBundleImage);
