...
const state = {
  ...
  catalogType:["product","bundle"].includes(new URLSearchParams(location.search).get("type")) ? new URLSearchParams(location.search).get("type") : "",
...
function mediaPreviewUrl(value, width = 160) {
...
function productCopyText(value = "") {
  const source = String(value ?? "").replace(/\\r\\n|\\n|\\r/g, "\n").replace(/\r\n?/g, "\n");
  if (!/<[a-z][^>]*>/i.test(source) && !/&lt;\/?[a-z]/i.test(source)) return source.trim();
...
function productImages(product) {
  const values = [product.main_photo_url || product.image_url, ...(product.side_photos || []), ...(product.gallery || []), ...activeVariants(product).flatMap((variant) => [variant.image_url, ...(variant.images || [])])].filter(Boolean);
...
function storefrontCategoryTree() {
  ...
  const alpha=(a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0)||(a.name_ar||a.name_en||"").localeCompare(b.name_ar||b.name_en||"","ar");
  const tree=rows.filter(category=>!category.parent_id).map(category=>({category,children:rows.filter(child=>Number(child.parent_id)===Number(category.id)).sort(alpha)}));
  const rank=entry=>entry.children.length?0:(String(entry.category.slug||"")==="best-sellers"||String(entry.category.name_en||"").toLowerCase()==="best sellers"?2:1);
  return tree.sort((a,b)=>rank(a)-rank(b)||alpha(a.category,b.category));
}
...
function productsMobileMenuHtml() {
  const entries=storefrontNavigationEntries();
  return `<div class="drawer-products" data-drawer-products><button class="drawer-products-toggle" type="button" aria-expanded="false"><span>المنتجات</span>${icon("chevron-down",18)}</button><div class="drawer-products-content" hidden>${entries.map(entry=>`<div class="drawer-category-item"><div class="drawer-category-row">${entry.children.length?`<button class="drawer-category-expand" type="button" data-drawer-category-toggle aria-expanded="false" aria-label="فتح التصنيفات الفرعية لـ ${esc(entry.title)}">${entry.image?`<img src="${esc(entry.image)}" alt="" />`:""}<span>${esc(entry.title)}</span>${icon("chevron-down",17)}</button>`:`<a href="${esc(entry.href)}">${entry.image?`<img src="${esc(entry.image)}" alt="" />`:""}<span>${esc(entry.title)}</span></a>`}</div>${entry.children.length?`<div class="drawer-subcategories" hidden><a class="drawer-category-all" href="${esc(entry.href)}"><span>كل ${esc(entry.title)}</span></a>${entry.children.map(child=>`<a href="${esc(storefrontCategoryHref(child,entry.source))}">${child.image_url?`<img src="${esc(child.image_url)}" alt="" />`:""}<span>${esc(child.name_ar||child.name_en||"")}</span></a>`).join("")}</div>`:""}</div>`).join("")}</div></div>`;
}

function bindProductsNavigation() {
  const root=document.querySelector("[data-products-nav]");
  if(root){
    const trigger=root.querySelector(".products-nav-trigger"),menu=root.querySelector(".products-mega-menu");
    const closeSubmenu=()=>{menu?.classList.remove("submenu-open");root.querySelectorAll("[data-products-category]").forEach(tab=>{tab.classList.remove("active");tab.setAttribute("aria-expanded","false");});root.querySelectorAll("[data-products-panel]").forEach(panel=>{panel.classList.remove("active");panel.hidden=true;});};
    const setExpanded=value=>{trigger?.setAttribute("aria-expanded",String(value));if(!value)closeSubmenu();};
    ...
    const activate=id=>{
      ...
      root.querySelectorAll("[data-products-category]").forEach(tab=>{const selected=String(tab.dataset.productsCategory)===String(id);tab.classList.toggle("active",selected);tab.setAttribute("aria-expanded",String(selected));});
    ...
    root.querySelectorAll(".products-category-tab:not([data-products-category])").forEach(tab=>{tab.addEventListener("mouseenter",closeSubmenu);tab.addEventListener("focus",closeSubmenu);});
  }
}
...
function bindDragScroll() {
  document.querySelectorAll("[data-drag-scroll]").forEach(rail=>{
    ...
    let touchStart=null;
    rail.addEventListener("touchstart",event=>{touchStart=event.touches.length===1?{x:event.touches[0].clientX,y:event.touches[0].clientY}:null;},{passive:true});
    rail.addEventListener("touchend",event=>{if(!touchStart)return;const touch=event.changedTouches[0];if(touch&&Math.abs(touch.clientX-touchStart.x)>12&&Math.abs(touch.clientX-touchStart.x)>Math.abs(touch.clientY-touchStart.y))suppressClickUntil=Date.now()+350;touchStart=null;},{passive:true});
    rail.addEventListener("touchcancel",()=>{touchStart=null;},{passive:true});
...
function bindAutoScroll() {
  document.querySelectorAll('[data-auto-scroll="true"]').forEach((rail) => {
    ...
    let paused = false, touchActive = false, resumeTimer = null;
    ...
    const resume = () => { if(touchActive)return; paused = false; position = rail.scrollLeft; normalizeLoop(); rail.scrollLeft = position; lastFrame = performance.now(); };
    const resumeAfterTouch = () => { clearTimeout(resumeTimer); resumeTimer = setTimeout(() => { if(!touchActive)resume(); }, 3500); };
    rail.addEventListener("pointerenter", pause);
    rail.addEventListener("pointerleave", event => { if(event.pointerType!=="touch")resume(); });
    rail.addEventListener("focusin", pause);
    rail.addEventListener("focusout", (event) => { if (!rail.contains(event.relatedTarget)) resume(); });
    rail.addEventListener("pointerdown", event => { pause(); if(event.pointerType==="touch")touchActive=true; });
    rail.addEventListener("pointerup", event => { if(event.pointerType==="touch"){touchActive=false;resumeAfterTouch();}else resume(); });
    rail.addEventListener("pointercancel", event => { if(event.pointerType==="touch"){touchActive=false;resumeAfterTouch();}else resume(); });
    rail.addEventListener("touchstart", () => { touchActive=true; pause(); }, {passive:true});
    rail.addEventListener("touchend", () => { touchActive=false; resumeAfterTouch(); }, {passive:true});
    rail.addEventListener("touchcancel", () => { touchActive=false; resumeAfterTouch(); }, {passive:true});
    const tick = (now) => {
...
function matchingCatalogEntries(omit="") {
  ...
  if(omit!=="catalogType"&&state.catalogType)rows=rows.filter(item=>item._catalog_type===state.catalogType);
...
function renderProducts() {
  ...
  const pageTitle=selectedSubcategory?.name_ar||selectedSubcategory?.name_en||selectedFacet?.name_ar||selectedFacet?.nameAr||selectedFacet?.name_en||selectedFacet?.nameEn||selectedCategory?.name_ar||selectedCategory?.name_en||(state.catalogType==="bundle"?"الأطقم":state.catalogType==="product"?"المنتجات":"المتجر");
...
function bindFilters() {
  ...
  document.querySelector("[data-clear-filters]")?.addEventListener("click",()=>{Object.assign(state,{catalogType:"",category:"",subcategory:"",facet:"",color:"",option:"",collection:"",minPrice:0,maxPrice:Infinity,page:1});updateUrl();renderProducts();});
...
function renderProduct(product) {
  const variants=activeVariants(product).sort((left,right)=>Number(variantInStock(right))-Number(variantInStock(left)));
  const cardPaymentEnabled=(state.paymentMethods?.methods||[]).some(method=>method.id==="edfapay");
  ...
  let quantity=1;
  const baseMedia=productMedia(product);
  const media=baseMedia;
  const initialImage=selectedVariant?.image_url || selectedVariant?.images?.[0] || media.find(item=>item.type === "image")?.url || product.main_photo_url;
  let mediaIndex=Math.max(0,media.findIndex(item=>item.url===initialImage));
  const mediaIsOutOfStock=()=>false;
  const updateMediaStockState=()=>{const stage=document.getElementById("productGalleryStage"),overlay=document.getElementById("productStockOverlay");stage?.classList.remove("is-out-of-stock");if(overlay)overlay.hidden=true;document.querySelectorAll("[data-gallery-index]").forEach(button=>button.classList.remove("is-out-of-stock"));};
  const thumbnail=(item,index)=>`<button class="gallery-thumb ${index===mediaIndex?"active":""} ${item.type==="video"?"is-video":""}" type="button" data-gallery-index="${index}" aria-label="${item.type==="video"?"تشغيل فيديو المنتج":`عرض صورة ${index+1}`}">${item.type==="video"?`<video src="${esc(item.url)}" muted preload="metadata" playsinline></video><span>${icon("play",17)}</span>`:`<img src="${esc(mediaPreviewUrl(item.url))}" alt="" loading="lazy" decoding="async" />`}</button>`;
  const productName=productCopyText(product.name_ar || product.name_en || "");
  const shortDescription=productCopyText(product.short_description_ar || "");
  const fullDescription=productCopyText(product.description_ar || product.short_description_ar || "");
  shell(`${breadcrumbs(productName)}<section class="container product-page"><div class="product-detail"><div class="product-gallery"><div class="gallery-thumbs" id="galleryThumbs">${media.map(thumbnail).join("")}</div><div class="gallery-main" id="productGalleryStage" aria-live="polite"><div id="galleryStageContent"></div><span class="product-stock-overlay" id="productStockOverlay" hidden>Out of Stock</span><button class="gallery-nav gallery-prev" id="galleryPrev" type="button" aria-label="الوسائط السابقة">${icon("chevron-left")}</button><button class="gallery-nav gallery-next" id="galleryNext" type="button" aria-label="الوسائط التالية">${icon("chevron-right")}</button><button class="zoom-hint" id="zoomProduct" type="button" aria-label="عرض بالحجم الكامل">${icon("maximize-2")}</button></div></div><div class="product-summary"><div class="product-meta">${esc(productCategoryName(product))}</div><h1>${esc(productName)}</h1>${shortDescription?`<p class="short-description">${esc(shortDescription)}</p>`:""}<div class="product-rating-summary" id="productRatingSummary" hidden></div><div class="price detail-price" id="detailPrice"></div><p class="tax-inclusive">السعر شامل الضريبة</p><div class="product-sales-proof" id="productSalesProof" hidden></div><div id="productPaymentWidgets" class="product-payment-widgets"></div><div id="variantControls"></div><p class="variant-stock-state" id="variantStockState" role="status" hidden>${icon("circle-alert",17)}نفدت كمية هذا الاختيار</p><div class="purchase-row"><div class="quantity-control"><button id="qtyPlus" aria-label="زيادة الكمية">+</button><strong id="qtyValue">1</strong><button id="qtyMinus" aria-label="تقليل الكمية">−</button></div><button class="primary-button" id="addProduct">${icon("shopping-cart")}إضافة إلى السلة</button></div><div class="product-buy-actions"><button class="secondary-button buy-now" id="buyNow">اشتري الآن</button><button class="secondary-button card-checkout" id="buyByCard" type="button" ${cardPaymentEnabled?"":'disabled title="الدفع بالبطاقة غير متاح حاليًا"'}>${icon("credit-card",18)}<span>${cardPaymentEnabled?"الدفع بالبطاقة":"البطاقة غير متاحة حاليًا"}</span></button></div><div class="product-trust"><span>${icon("shield-check",18)}دفع آمن وبيانات محمية</span><span>${icon("badge-check",18)}منتج أصلي من رداء الحشمة</span></div></div></div>${fullDescription?`<section class="detail-description"><h2>وصف المنتج</h2><div class="rich-description product-copy-text">${esc(fullDescription)}</div></section>`:""}<section class="product-reviews-root" id="productReviewsRoot" data-product-id="${esc(product.id)}" aria-live="polite"><div class="reviews-loading" aria-label="جاري تحميل التقييمات"><span></span><span></span><span></span></div></section></section><section class="section soft"><div class="container"><div class="section-head"><h2>منتجات قد تعجبك</h2></div><div class="product-grid">${state.products.filter(item=>item.id!==product.id).slice(0,4).map(productCard).join("")}</div></div></section>`);
  ...
  const openCurrentMedia=()=>{
    ...
    const draw=()=>{const item=media[mediaIndex],target=document.getElementById("lightboxMedia");if(!target)return;target.innerHTML=item.type==="video"?`<video src="${esc(item.url)}" controls autoplay playsinline></video>`:`<span class="lightbox-loading" role="status">جاري تحميل الصورة الأصلية...</span><img src="${esc(item.url)}" alt="${esc(product.name_ar)}" style="display:none" />`;if(item.type!=="video"){const image=target.querySelector("img"),show=()=>{image.style.display="block";target.querySelector(".lightbox-loading")?.remove();};image.onload=show;image.onerror=()=>{target.querySelector(".lightbox-loading").textContent="تعذر تحميل الصورة";};if(image.complete&&image.naturalWidth)show();}document.getElementById("lightboxCounter").textContent=`${mediaIndex+1} / ${media.length}`;};
  ...
  const renderMedia=(nextIndex)=>{
    ...
    stage.innerHTML=item.type==="video"?`<video id="mainProductVideo" src="${esc(item.url)}" controls preload="metadata" playsinline></video>`:`<img id="mainProductImage" src="${esc(mediaPreviewUrl(item.url,960))}" alt="${esc(product.name_ar)}" draggable="false" decoding="async" /><span class="image-zoom-lens" id="imageZoomLens" aria-hidden="true"></span>`;
    ...
    if(item.type==="image"&&matchMedia("(hover:hover) and (pointer:fine)").matches){
      ...
      stage.onpointermove=event=>{const rect=image.getBoundingClientRect();const x=Math.max(0,Math.min(rect.width,event.clientX-rect.left));const y=Math.max(0,Math.min(rect.height,event.clientY-rect.top));lens.style.left=`${x}px`;lens.style.top=`${y}px`;lens.style.backgroundImage=`url("${String(mediaPreviewUrl(item.url,960)).replaceAll('"','%22')}")`;lens.style.backgroundSize=`${rect.width*2.4}px ${rect.height*2.4}px`;lens.style.backgroundPosition=`${-(x*2.4-lens.offsetWidth/2)}px ${-(y*2.4-lens.offsetHeight/2)}px`;lens.classList.add("visible");};
  ...
  bindGalleryThumbs();
  applyVariantMedia(selectedVariant);
  ...
  const update=()=>{
    const colorAvailable=color=>variants.some(variant=>variant.color===color.name&&variantInStock(variant));
    const colorRows=uniqueColors(product).sort((left,right)=>Number(colorAvailable(right))-Number(colorAvailable(left)));
    const valueRows=variants.filter(variant=>!selectedColor||variant.color===selectedColor);
    const valueAvailable=value=>valueRows.some(variant=>variant.value===value&&variantInStock(variant));
    const values=[...new Set(valueRows.map(variant=>variant.value).filter(Boolean))].sort((left,right)=>Number(valueAvailable(right))-Number(valueAvailable(left)));
    ...
    const unavailable=!productInStock(product)||Boolean(variant&&!variantInStock(variant)),stockState=document.getElementById("variantStockState"),addButton=document.getElementById("addProduct"),buyButton=document.getElementById("buyNow"),cardButton=document.getElementById("buyByCard");
    stockState.hidden=!unavailable;stockState.innerHTML=unavailable?`${icon("circle-alert",17)}${variant?"نفدت كمية هذا الاختيار":"نفدت كمية المنتج"}`:"";addButton.disabled=unavailable;buyButton.disabled=unavailable;cardButton.disabled=unavailable||!cardPaymentEnabled;addButton.innerHTML=unavailable?`${icon("circle-x",18)}نفدت الكمية`:`${icon("shopping-cart")}إضافة إلى السلة`;buyButton.textContent=unavailable?"هذا الاختيار غير متاح":"اشتري الآن";
  ...
  document.getElementById("buyByCard").onclick=async()=>{if(!cardPaymentEnabled)return;const variant=currentVariant();if(!productInStock(product)||(variant&&!variantInStock(variant))){toast("نفدت كمية هذا الاختيار");return;}const button=document.getElementById("buyByCard");button.disabled=true;if(await addToCart(product,variant,quantity,false))location.href="/checkout?payment=edfapay";else button.disabled=false;};
  hydrateIcons();
...
function renderBundle(bundle) {
  ...
  const colorKey=option=>String(option?.color_id||option?.color_name_ar||option?.color_name_en||option?.color||"default");
  ...
  const draw=()=>{
    ...
    const colors=[...new Map(options.map(option=>[colorKey(option),option])).values()];
    const colorButtons=colors.map(option=>{const variants=options.filter(item=>colorKey(item)===colorKey(option)),soldOut=variants.every(isSoldOutOption);return `<button type="button" data-bundle-color="${esc(colorKey(option))}" class="${colorKey(option)===colorKey(selected)?"active":""} ${soldOut?"is-out-of-stock":""}" >${option.hex_code?`<i style="--bundle-color:${esc(option.hex_code)}"></i>`:""}<b>${esc(option.color_name_ar||option.color||"طقم")}</b><small>${soldOut?"نفد":`${variants.length} اختيارات`}</small></button>`;}).join("");
    const availableForColor=options.filter(option=>colorKey(option)===colorKey(selected));
    const optionButtons=availableForColor.map(option=>{const soldOut=isSoldOutOption(option);return `<button type="button" data-bundle-option="${esc(option.id)}" class="${String(option.id)===String(selected?.id)?"active":""} ${soldOut?"is-out-of-stock":""}" ${soldOut?"disabled":""}><b>${esc(option.option_name_ar||option.value||option.label_ar||"اختيار")}</b><small>${soldOut?"نفد":money(option.price)}</small></button>`;}).join("");
    const optionMedia=option=>[option?.image_url,...(Array.isArray(option?.images)?option.images:[])].map(entry=>String(entry?.url||entry||"").trim()).filter(Boolean).map(url=>({url,label:option?.label_ar||option?.label_en||bundle.name_ar}));
    const galleryCandidates=[
      ...optionMedia(active),
      {url:bundle.main_photo_url||bundle.image_url,label:bundle.name_ar||bundle.name_en},
      ...(Array.isArray(bundle.side_photos)?bundle.side_photos:[]).map(url=>({url,label:bundle.name_ar||bundle.name_en})),
      ...items.map(item=>({url:item.image_url,label:item.name_ar||item.name_en||bundle.name_ar})),
      ...options.flatMap(optionMedia)
    ];
    const seenImages=new Set(),gallery=galleryCandidates.filter(item=>{const url=String(item.url||"").trim();if(!url||seenImages.has(url))return false;seenImages.add(url);item.url=url;return true;});
    if(mediaIndex>=gallery.length)mediaIndex=0;
    const galleryHtml=gallery.length?`<div class="product-gallery bundle-gallery"><div class="gallery-thumbs" id="bundleGalleryThumbs">${gallery.map((item,index)=>`<button class="gallery-thumb ${index===mediaIndex?"active":""}" type="button" data-bundle-gallery-index="${index}" aria-label="عرض ${esc(item.label||`صورة ${index+1}`)}"><img src="${esc(mediaPreviewUrl(item.url))}" alt="" loading="lazy" decoding="async" /></button>`).join("")}</div><div class="gallery-main" id="bundleGalleryStage" aria-live="polite"><div id="bundleGalleryStageContent"></div>${gallery.length>1?`<button class="gallery-nav gallery-prev" id="bundleGalleryPrev" type="button" aria-label="الصورة السابقة">${icon("chevron-left")}</button><button class="gallery-nav gallery-next" id="bundleGalleryNext" type="button" aria-label="الصورة التالية">${icon("chevron-right")}</button>`:""}<button class="zoom-hint" id="zoomBundleImage" type="button" aria-label="عرض الصورة الأصلية">${icon("maximize-2")}</button></div></div>`:`<div class="bundle-main-visual">${bundleVisual({...bundle,items})}</div>`;
    const configuratorHtml=`<div class="bundle-configurator bundle-configurator-inline" id="bundleConfigurator"><header><span>اختيارات الطقم</span><h2>اختاري اللون ونوع الشرشف</h2><p>اللون المختار يطبّق على السجادة والشرشف معًا، ثم حددي خياطة الشرشف المناسبة.</p></header>${options.length?`<div class="bundle-option-picker bundle-color-picker"><span>اللون</span><div>${colorButtons}</div></div><div class="bundle-option-picker"><span>الخيار المتاح لهذا اللون</span><div>${optionButtons}</div></div>`:""}<div class="bundle-purchase-actions"><div class="purchase-row"><div class="quantity-control"><button id="bundleQtyPlus" aria-label="زيادة الكمية">+</button><strong id="bundleQtyValue">${quantity}</strong><button id="bundleQtyMinus" aria-label="تقليل الكمية">−</button></div><button class="primary-button" id="addBundle" ${active.available_stock===0?"disabled":""}>${icon("shopping-cart")}${active.available_stock===0?"نفد هذا الاختيار":"إضافة الطقم للسلة"}</button></div><button class="secondary-button buy-now" id="buyBundleNow" ${active.available_stock===0?"disabled":""}>اشتري الآن</button></div></div>`;
    shell(`${breadcrumbs(bundle.name_ar||"أطقم المنتجات")}<section class="container product-page bundle-page"><div class="product-detail">${galleryHtml}<div class="product-summary"><div class="product-meta">طقم خاص · ${Number(active.item_count||items.length||0)} قطع</div><h1>${esc(bundle.name_ar||bundle.name_en)}</h1><div class="price detail-price">${compare>price?`<del>${money(compare)}</del>`:""}<strong>${money(price)}</strong></div>${saving?`<div class="bundle-saving">وفّري ${money(saving)} عند شراء الطقم</div>`:""}${bundle.sales_proof?.visible?`<div class="product-sales-proof">${icon("trending-up",16)}<span>${esc(bundle.sales_proof.label?.ar||"")}</span></div>`:""}${shortDescription?`<p class="short-description">${esc(shortDescription)}</p>`:""}${configuratorHtml}</div></div><section class="bundle-includes"><div class="section-head center"><h2>الطقم يحتوي على</h2></div><div class="bundle-items-row">${items.map((item,index)=>`${index?`<span class="bundle-item-plus">+</span>`:""}<a class="bundle-item-card" href="/product/${item.product_id}"><img src="${esc(mediaPreviewUrl(item.image_url))}" alt="${esc(item.name_ar)}" loading="lazy" decoding="async" /><div><span>${item.quantity>1?`${item.quantity} × `:""}منتج #${item.product_id}</span><strong>${esc(item.name_ar||item.name_en)}</strong>${item.variant_label?`<em>${esc(item.variant_label)}</em>`:""}<small>${money(item.unit_price)}</small></div></a>`).join("")}</div></section>${fullDescription?`<section class="detail-description"><h2>وصف الطقم</h2><div class="rich-description">${fullDescription}</div></section>`:""}</section>`);
    const showMedia=index=>{
      if(!gallery.length)return;
      mediaIndex=(Number(index)+gallery.length)%gallery.length;
      const item=gallery[mediaIndex],stage=document.getElementById("bundleGalleryStageContent");
      if(!stage)return;
      stage.innerHTML=`<img id="bundleGalleryImage" src="${esc(mediaPreviewUrl(item.url,960))}" alt="${esc(item.label||bundle.name_ar||"")}" draggable="false" decoding="async" /><span class="image-zoom-lens" id="bundleImageZoomLens" aria-hidden="true"></span>`;
      const image=stage.querySelector("img"),lens=stage.querySelector(".image-zoom-lens");
      image.onclick=openBundleImage;
      if(matchMedia("(hover:hover) and (pointer:fine)").matches){
        stage.onpointermove=event=>{const rect=image.getBoundingClientRect(),x=Math.max(0,Math.min(rect.width,event.clientX-rect.left)),y=Math.max(0,Math.min(rect.height,event.clientY-rect.top));lens.style.left=`${x}px`;lens.style.top=`${y}px`;lens.style.backgroundImage=`url("${String(mediaPreviewUrl(item.url,960)).replaceAll('"','%22')}")`;lens.style.backgroundSize=`${rect.width*2.4}px ${rect.height*2.4}px`;lens.style.backgroundPosition=`${-(x*2.4-lens.offsetWidth/2)}px ${-(y*2.4-lens.offsetHeight/2)}px`;lens.classList.add("visible");};
        stage.onpointerleave=()=>lens.classList.remove("visible");
      }else{stage.onpointermove=null;stage.onpointerleave=null;}
      document.querySelectorAll("[data-bundle-gallery-index]").forEach((button,buttonIndex)=>button.classList.toggle("active",buttonIndex===mediaIndex));
      document.querySelector(`[data-bundle-gallery-index="${mediaIndex}"]`)?.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"});
    };
    const openBundleImage=()=>{
      if(!gallery[mediaIndex])return;
      openOverlay(`<div class="lightbox" role="dialog" aria-modal="true" aria-label="معرض صور الطقم"><button class="close-button" data-overlay-close aria-label="إغلاق">${icon("x")}</button><div class="lightbox-media" id="bundleLightboxMedia"></div>${gallery.length>1?`<button class="lightbox-nav lightbox-prev" type="button" aria-label="الصورة السابقة">${icon("chevron-left",28)}</button><button class="lightbox-nav lightbox-next" type="button" aria-label="الصورة التالية">${icon("chevron-right",28)}</button>`:""}<span class="lightbox-counter" id="bundleLightboxCounter"></span></div>`);
      const drawLightbox=()=>{const item=gallery[mediaIndex],target=document.getElementById("bundleLightboxMedia");if(!target)return;target.innerHTML=`<span class="lightbox-loading" role="status">جاري تحميل الصورة الأصلية...</span><img src="${esc(item.url)}" alt="${esc(item.label||bundle.name_ar||"")}" style="display:none" />`;const image=target.querySelector("img"),show=()=>{image.style.display="block";target.querySelector(".lightbox-loading")?.remove();};image.onload=show;image.onerror=()=>{target.querySelector(".lightbox-loading").textContent="تعذر تحميل الصورة";};if(image.complete&&image.naturalWidth)show();document.getElementById("bundleLightboxCounter").textContent=`${mediaIndex+1} / ${gallery.length}`;};
      const move=direction=>{showMedia(mediaIndex+direction);drawLightbox();};
      overlayRoot.querySelector(".lightbox-prev")?.addEventListener("click",()=>move(-1));
      overlayRoot.querySelector(".lightbox-next")?.addEventListener("click",()=>move(1));
      let startX=null;
      overlayRoot.querySelector(".lightbox-media")?.addEventListener("touchstart",event=>{startX=event.touches[0]?.clientX??null;},{passive:true});
      overlayRoot.querySelector(".lightbox-media")?.addEventListener("touchend",event=>{if(startX===null)return;const endX=event.changedTouches[0]?.clientX??startX;if(Math.abs(startX-endX)>48)move(startX>endX?1:-1);startX=null;},{passive:true});
      drawLightbox();hydrateIcons();
    };
    showMedia(mediaIndex);
    document.getElementById("zoomBundleImage")?.addEventListener("click",openBundleImage);
    document.querySelectorAll("[data-bundle-gallery-index]").forEach(button=>button.onclick=()=>showMedia(Number(button.dataset.bundleGalleryIndex)));
    document.getElementById("bundleGalleryPrev")?.addEventListener("click",()=>showMedia(mediaIndex-1));
    document.getElementById("bundleGalleryNext")?.addEventListener("click",()=>showMedia(mediaIndex+1));
    let touchStart=null;
    document.getElementById("bundleGalleryStage")?.addEventListener("touchstart",event=>{touchStart=event.touches[0]?.clientX??null;},{passive:true});
    document.getElementById("bundleGalleryStage")?.addEventListener("touchend",event=>{if(touchStart===null)return;const end=event.changedTouches[0]?.clientX??touchStart;if(Math.abs(touchStart-end)>48)showMedia(mediaIndex+(touchStart>end?1:-1));touchStart=null;},{passive:true});
    document.querySelectorAll("[data-bundle-option]").forEach(button=>button.onclick=()=>{selected=options.find(option=>String(option.id)===button.dataset.bundleOption)||selected;quantity=1;mediaIndex=0;draw();requestAnimationFrame(()=>document.getElementById("bundleConfigurator")?.scrollIntoView({block:"nearest"}));});
    document.querySelectorAll("[data-bundle-color]").forEach(button=>button.onclick=()=>{const matching=options.filter(option=>colorKey(option)===button.dataset.bundleColor);selected=matching.find(option=>!isSoldOutOption(option))||matching[0]||selected;quantity=1;mediaIndex=0;draw();});
...
function checkoutFormMarkup(countries, defaultCountry) {
  ...
  const methods=state.paymentMethods?.methods||[];
  const requestedPayment=new URLSearchParams(location.search).get("payment");
  const selectedPayment=methods.some(method=>method.id===requestedPayment)?requestedPayment:methods[0]?.id;
  const cardBrands=[
    ["mada.webp","مدى","mada"],
    ["visa.svg","Visa","visa"],
  ...
  const paymentChoices=methods.map((method,index)=>{
    ...
    return `<label class="checkout-payment-choice is-${esc(method.id)}"><input type="radio" name="payment_method" value="${esc(method.id)}" ${method.id===selectedPayment?"checked":""} required /><span class="checkout-payment-indicator"></span><span class="checkout-payment-frame">${content}</span></label>`;
  }).join("");
