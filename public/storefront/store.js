...
function renderBundle(bundle) {
  ...
  const draw=()=>{
    ...
    const galleryHtml=gallery.length?`<div class="product-gallery bundle-gallery"><div class="gallery-thumbs" id="bundleGalleryThumbs">${gallery.map((item,index)=>`<button class="gallery-thumb ${index===mediaIndex?"active":""}" type="button" data-bundle-gallery-index="${index}" aria-label="عرض ${esc(item.label||`صورة ${index+1}`)}"><img src="${esc(item.url)}" alt="" loading="lazy" /></button>`).join("")}</div><div class="gallery-main" id="bundleGalleryStage"><div id="bundleGalleryStageContent"><img id="bundleGalleryImage" src="${esc(gallery[mediaIndex]?.url||"")}" alt="${esc(gallery[mediaIndex]?.label||bundle.name_ar||"")}" /></div>${gallery.length>1?`<button class="gallery-nav gallery-prev" id="bundleGalleryPrev" type="button" aria-label="الصورة السابقة">${icon("chevron-left")}</button><button class="gallery-nav gallery-next" id="bundleGalleryNext" type="button" aria-label="الصورة التالية">${icon("chevron-right")}</button>`:""}</div></div>`:`<div class="bundle-main-visual">${bundleVisual({...bundle,items})}</div>`;
